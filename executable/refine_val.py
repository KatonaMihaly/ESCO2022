...
if switch == 1:
    ...
    angle = [[] for i in range(len(prod2))]
    a = 0
    b = 0
    while a < len(prod2):
        t[a] = (torque["Torque"])[b]
    ...
    for i in range(len(prod2)):
        angle[i] = np.multiply(prod2[i], 4)

    case = {"earheight": [(prod2[i])[0] for i in range(len(prod2))],
            "angle": [(angle[i])[1] for i in range(len(prod2))],
            "torque": t}
    ...
    ax.set_xlabel('Electrical angle [deg]', fontsize=10)
    ...
    plt.savefig(ModelDir.MEDIA / "PEMC_3D1.png", bbox_inches="tight", dpi=650)
    plt.show()

elif switch == 4:
    t = [[] for i in range(len(prod3))]
    angle = [[] for i in range(len(prod3))]
    a = 0
    b = 45136
    while a < len(prod3):
        t[a] = (torque["Torque"])[b]
    ...
    for i in range(len(prod3)):
        angle[i] = np.multiply(prod3[i], 4)

    case = {"aslheight": [(prod3[i])[0] for i in range(len(prod3))],
            "angle": [(angle[i])[1] for i in range(len(prod3))],
            "torque": t}
    ...
    fig = plt.figure(figsize=(6, 6))
    ax = fig.add_subplotplot(projection='3d')
    a = 56
    b = 79
